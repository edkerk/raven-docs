...
`yeast-GEM.xml` from [`docs/data/`](../data/README.md) â€” yeast-GEM v9.1.0, which
...
=== "MATLAB"
    ...
    `sol.f` is the objective value itself â€” no sign to undo, whatever RAVEN does
    internally â€” and `sol.x` holds the flux vector, in the order of `model.rxns`.
...
The objective is a reaction to maximise â€” growth, a product exchange, an ATP
demand.
...
the way out. MATLAB has no equivalent â€” copy the struct, change the copy, and let
it go out of scope.
...
=== "Python"
    ...
    not enough to make yeast-GEM grow fermentatively â€” it also needs sterol and
...
=== "Python"
    ...
    `model.summary()` prints the same picture â€” uptake, secretion and the
    objective â€” as a table, and `solution.fluxes` is a pandas Series, so the usual
...
=== "Python"
    ...
    achieves it. The *number* of active reactions still varies between solves â€”
    pFBA pins the total flux, not which reactions carry it â€” so do not build a
    test on that count.

!!! warning "What can go wrong"
    - **The status is `infeasible`.** Something is over-constrained â€” most often
    ...
    - **Growth is zero but the solve succeeded.** The model is feasible and the
      ...
      pseudoreaction.
    - **The same model gives different flux distributions.** Expected â€” alternative
...
- [Getting started](getting-started.md) â€” loading and inspecting a model.
- [Growth media and conditions](media.md) â€” what the exchange bounds mean and how
  to define a medium.
