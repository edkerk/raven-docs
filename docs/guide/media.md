...
=== "Python"
    ...
    **positive** number â€” cobrapy flips the sign for you, so a medium entry of
...
!!! note "270 or 273?"
    ...
    difference is three reactions â€” `r_2111` (growth) and the sinks `r_4062` and
    `r_4064` â€” so MATLAB reports 273 where `model.exchanges` reports 270.
...
=== "Python"
    ...
    Glucose and oxygen alone are not enough â€” yeast-GEM also needs nitrogen,
...
YAML file â€” a *condition* â€” which can be reviewed as data rather than buried in a
script.
...
=== "MATLAB"
    ...
    MATLAB's Python bridge â€” so it needs a linked CPython with `pyyaml`
...
uptakes that still supports a given growth rate â€” useful to find out which
...
=== "MATLAB"
    ...
    cannot do â€” with GLPK selected it reports `glpk is not suitable for solving
...
- [Simulating growth with FBA](fba.md) â€” the solve this page feeds.
- [Reading and writing models](io.md) â€” keeping a model and its conditions in a
  repository.
