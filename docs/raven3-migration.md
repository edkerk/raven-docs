...
!!! note "This is the MATLAB-to-MATLAB axis"
    This page is about RAVEN 2 → RAVEN 3, both MATLAB. For how the Python
    package differs from RAVEN 2.0, which is a separate question, see
    [Changes from RAVEN 2](raven2-changes.md).
...
| Nearest replacement for curation | — | `curation/curateModelFromTables.m`: curates/extends an *existing* model from separate `.tsv` files (mets by name+compartment, reactions by stoichiometry, genes by name) — not a from-scratch importer |
